...
void solve() {
    ...
    forn(i, n) {
        cin >> a[i];
        int inv = 0;
        forn(j, len(a[i])) forn(k, j) inv += a[i][j] < a[i][k];
        if (inv % 2 == 0) mf.add_edge(s, i);
        else mf.add_edge(i, t);
        forn(j, i) {
            int x = 0;
            forn(k, len(a[i])) x += a[i][k] != a[j][k];
            if (x == 2) {
                if (inv % 2 == 0) mf.add_edge(i, j);
                else mf.add_edge(j, i);
            }
        }
    }
    cout << n - mf.max_flow(s, t) << "\n";
}
