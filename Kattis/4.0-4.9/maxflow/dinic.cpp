...
struct MF {
    ...
    vector<tuple<int, int, ll>> max_flow_assignment(int s, int t) {
        dinic(s, t);
...
int main() {
    ...
    vector<tuple<int, int, ll>> used_flows = mf.max_flow_assignment(s, t);

    print(n, mf.mf, len(used_flows));
    for (auto [u, v, f] : used_flows) {
        print(u, v, f);
    }
}
